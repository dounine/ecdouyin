...
class ChromePools(system: ActorSystem[_]) extends Extension {
  ...
  private val poolConfig: GenericObjectPoolConfig[ChromeResource] = new GenericObjectPoolConfig()
  ...
  poolConfig.setMinEvictableIdleTimeMillis(config.getInt("selenium.pool.minEvictableIdleTimeMillis"))
  poolConfig.setTimeBetweenEvictionRunsMillis(config.getInt("selenium.pool.timeBetweenEvictionRunsMillis"))
  poolConfig.setBlockWhenExhausted(config.getBoolean("selenium.pool.blockWhenExhausted"))

  private val chromeFactory = new ChromeFactory(system)
  private val _pools = new GenericObjectPool[ChromeResource](chromeFactory, poolConfig)

  def pools: GenericObjectPool[ChromeResource] = _pools
