...
object QrcodeUnIdentifyStatus extends JsonParse {

  private final val logger: Logger =
    LoggerFactory.getLogger(QrcodeUnIdentifyStatus.getClass)
  ...
  def apply(
      ...
      command match {
        ...
        case e @ CreateOrder(_) => {
          ...
          Effect.none.thenRun((latest: State) => {
            e.replyTo.tell(
              CreateOrderFail(e, "qrcodeUnIdentify")
            )
          })
        }
    ...
    (
      commandHandler,
      defaultEvent,
      classOf[QrcodeUnIdentify],
      MechineStatus.qrcodeUnIdentify
    )
