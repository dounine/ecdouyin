...
object QrcodeSources {
  ...
  ): Source[Either[Throwable, ChromeResource], NotUsed] = {
      ...
      .flatMapConcat { source =>
        {
          Source
                    ...
                    .mapAsync(1) {
                      ...
                      case Right(_) =>
                        Future {
                          ...
                          case _ => throw new Exception("二维码保存失败")
                        }
                    }
                }
            }
            .map(file =>
              Right(new {
                val source: ChromeResource = source
                val order: OrderModel.DbInfo = order
                val qrcode: String = file.getAbsolutePath
              })
            )
            .recover {
              case e: Throwable => Left(e)
            ...
            .watchTermination()((pv, future) => {
              future.foreach(_ => {
                logger.info("chrome source 资源回收")
